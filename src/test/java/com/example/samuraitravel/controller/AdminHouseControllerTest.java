...
@ActiveProfiles("test") //テスト時に設定したいファイルapplication-◯◯◯.propertiesの部分を指定する
public class AdminHouseControllerTest {
	...
	@Test
	@WithUserDetails("taro.samurai@example.com") //ログイン済みのユーザーとして扱う
	public void 一般ユーザーとしてログイン済みの場合は管理者用の民宿一覧ページが表示されずに403エラーが発生する() throws Exception {
		mockMvc.perform(get("/admin/houses"))      //HTTPリクエストを送信
		       .andExpect(status().isForbidden()); //HTTPレスポンスの内容を検証する
	}
	...
	@Test
	public void 未ログインの場合が管理者用の民宿登録ページからログインページにリダイレクトする() throws Exception {
